...
def print_summary(downloaded_items, skipped_items, failed_items):
    """Prints execution summary, listing downloaded, skipped, and failed items
    separately."""
    logger.info('\n\n' + '=' * 80)
    ...
    if downloaded_items:
        logger.info(
            f'Successfully downloaded: {len(downloaded_items)} item(s)')
    ...
    if skipped_items:
        logger.info(
            f'Already downloaded and skipped: {len(skipped_items)} item(s)\n')
    ...
    if not failed_items:
        ...
        for idx, item in enumerate(failed_items, 1):
            ...
            logger.error(
                f'       Wget Code:   {item.get("error_code", "Unknown")}')
...
def process_model(model, checkpoint_dir, downloaded_items, skipped_items,
                  failed_items):
    """Filters model entry and handles download/skip/failure logic for its
    weights."""
    ...
    item_info = {'name': name, 'url': weights_url, 'filename': filename}
...
def main():
    ...
    for mf_path in metafiles:
        ...
        for model in data['Models']:
            process_model(model, checkpoint_dir, downloaded_items,
