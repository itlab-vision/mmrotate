...
class RestartOnNanHook(Hook):
    ...
    def before_run(self, runner):
        ...
        self.initial_state = {
            'model':
            {k: v.cpu().clone()
             for k, v in runner.model.state_dict().items()},
            'optimizer': copy.deepcopy(runner.optimizer.state_dict())
        }

        # Monkey-patch runner.train to allow aborting
        # the epoch without crashing the process
        ...
        def safe_train(*args, **kwargs):
            try:
                ...
                if str(e) == 'NaN_ABORT_EPOCH':
                    runner.logger.info(
                        'Epoch aborted successfully. The next loop will '
                        'restart from the restored epoch 0-th batch.')
                else:
                    raise e
    ...
    def after_train_iter(self, runner):
        # 1. Check for NaN/Inf locally in log_vars
        log_vars = runner.outputs.get('log_vars', {})
        is_nan = any(
            math.isnan(val) or math.isinf(val) for val in log_vars.values())

        if is_nan:
            runner.logger.error(
                f'NaN/Inf detected on rank {runner.rank} '
                f'at epoch {runner.epoch}, iter {runner.inner_iter}. '
                f'Log vars: {log_vars}')

        # 2. Synchronize the NaN flag across all GPUs (if distributed)
        rank, world_size = get_dist_info()

        is_nan_tensor = torch.tensor(
            int(is_nan), dtype=torch.int32, device='cuda')
        if world_size > 1:
        ...
        if is_nan_tensor.item() > 0:
            if rank == 0:
                runner.logger.warning('NaN detected on at least one GPU! '
                                      'Synchronously restoring checkpoint...')
