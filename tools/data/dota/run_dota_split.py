...
def parse_args():
    parser = argparse.ArgumentParser(description='DOTA dataset splitting wrapper.')
    parser.add_argument(
        '--dota-version', 
        nargs='+', 
    ...
    parser.add_argument(
        ...
        choices=['ss', 'ms', 'all'], 
        default=['ss', 'ms'],
        help='Scale mode(s): single-scale (ss) or multi-scale (ms) (default: ss ms)')
    parser.add_argument(
        '--nproc', 
        type=int, 
        default=10,
...
def run_split_command(config, version, split, scale, overwrite):
    ...
    if not is_valid:
        logger.info(f"[-] Skipping {version} | {split} | {scale.upper()} -> {reason}")
        return
    ...
    if os.path.exists(save_dir):
        if overwrite:
            ...
            logger.info(f"[-] Skipping {version} | {split} | {scale.upper()} -> Directory '{save_dir}' exists (use --overwrite).")
            return
...
def main():
    ...
    for version in versions:
        for split in splits:
            for scale in scales:
                config = generate_split_config(version, split, scale, args.nproc)
                run_split_command(config, version, split, scale, args.overwrite)
